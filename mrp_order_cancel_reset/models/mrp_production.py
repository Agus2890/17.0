    ...
    def action_cancel_draft(self):
        ...
        for rec in self:
            if rec.state not in ["confirmed", "cancel"]:
                ...
                # Cancela los movimientos relacionados
                moves = rec.move_raw_ids | rec.move_finished_ids
                moves._action_cancel()
                moves.write({"state": "draft"})


                # Reinicia el estado de las órdenes de trabajo si existen
                rec.workorder_ids = [
                    Command.update(wo.id, {'state': 'waiting',"working_state": "normal",'production_state':'draft','is_user_working':False})
                    for wo in rec.workorder_ids
                ]
                # Restablece los valores que influyen en el estado
                rec.write({
                    "state": "draft",
                    "is_locked": False,
                    "qty_producing": 0.0,  # Evitar condiciones de progreso/to_close
                })
